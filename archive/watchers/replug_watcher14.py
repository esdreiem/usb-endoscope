#!/usr/bin/env python3
"""Replug capture #14: real MFi challenge + TWO-param 0xAE01 EA-session grant.
...
_dump = {"f": open("tools/replug14_ae_dump.bin", "wb"), "n": 0}
    ...
    with _fr_lock:
        ...
        while i < len(payload):
            if not _fr["in"]:
                ...
                frames["n"] += 1
                if frames["n"] <= 6:
                    fn = f"tools/replug14_frame_{frames['n']}.jpg"
                    open(fn, "wb").write(bytes(_fr["buf"]))
                    log(f"** FRAME {frames['n']} ({src}): {len(_fr['buf'])}B -> {fn}")
                _fr["in"] = False; _fr["buf"] = bytearray()

# #14 nutzt das nach dem #13-Reboot bereits offene Detect-Fenster: Gerät ist
# schon da und funkt Beats. Kein Warten auf Ab-/Anstecken.
dev = find_dev()
if dev is None:
    log("Gerät nicht da — warte auf Anstecken ...")
    while dev is None:
        dev = find_dev(); time.sleep(0.15)
log("Gerät gefunden, claim If0")
...
# ---- Handshake OHNE Challenge: 0xAA02 lässt diesen Klon abstürzen und neu
# enumerieren (Capture #13 — canned Apple-Cert, aber keine funktionierende
# Auth-IC). Accessory-seitig ist AA00 -> AA05 spec-legal (siehe research/).
...
time.sleep(0.5)
_dump["f"].close()
log(f"ENDE: {frames['n']} Frames; AE00/AE02 gedumpt: {_dump['n']} (-> tools/replug14_ae_dump.bin); "
